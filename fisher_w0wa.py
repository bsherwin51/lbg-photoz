...
def fiducial_model(theta):
    ...
    Omega_M, sigma8, Omega_b, h, n_s, w0, wa, bu, bg, br, bu_int, bg_int, br_int = theta 
    cosmo = MainCosmology(Omega_M=Omega_M, sigma8 = sigma8, Omega_b = Omega_b, h = h, n_s = n_s, w0=w0, wa=wa)
...
if __name__ == "__main__":
    ...
    theta0 = np.array([0.3156, 0.831, 0.0491685, 0.6727, 0.9645, -1, 0, 2.1574085950258355, 3.3278572844505083, 4.858283802099753, 1.49, 1.69, 1.79])  
    cosmo = MainCosmology(Omega_M=theta0[0], sigma8=theta0[1], Omega_b=theta0[2], h=theta0[3], n_s=theta0[4], w0=theta0[5], wa=theta0[6])
    ...
    for mapper in base_mappers:
        ...
        if mapper.drop_band == "u":
            ...
            mapper.f_interlopers = 0.03 # Petri Y10
            mapper.g_bias = theta0[9]
            mapper.g_bias_inter = theta0[12]
    ...
    np.save("data_cov_petri_y10_w0wa.npy", cov)
    ...
    np.save("fisher_petri_y10_w0wa.npy", fisher)
    ...
    print(f"{'Scenario':<15} | {'Sig d_w0':<8} | {'Sig d_wa':<8}")
    print("-" * 40)
    ...
    for s in scenarios:
        ...
        print(f"{s['name']:<15} | {delta_sigma[5]:.2e} | {delta_sigma[6]:.2e}")

    np.save("raw_shifts_w0wa_petri_y10.npy", raw_shifts)
    np.save("sigma_shifts_w0wa_petri_y10.npy", sigma_shifts)
